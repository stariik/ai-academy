...
export function ContentSheet(props: Props) {
  ...
  const body = (
    <>
      <SheetHeader {...props} subView={subView} onJumpToMaterial={() => setSubView('material')} />
      <div className="flex-1 overflow-y-auto">
        <AnimatePresence mode="wait" initial={false}>
          {subView === 'material' ? (
            ...
            >
              <ChecksView {...props} onBack={() => setSubView('material')} />
            </motion.div>
          )}
        </AnimatePresence>
      </div>
  ...
  return (
    <motion.div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="გაკვეთილის კონტენტი"
    >
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
...
function SheetHeader({
  ...
  subView,
  onJumpToMaterial,
}: Props & { subView: SubView; onJumpToMaterial: () => void }) {
  return (
    <div className="shrink-0 px-4 sm:px-5 pt-3 pb-3 border-b border-border bg-background/95 backdrop-blur-sm">
      <div className="flex items-center justify-between gap-3 mb-2">
        <div className="flex items-center gap-2">
          ...
          {/* Sub-step indicator */}
          <SubViewBadge subView={subView} onJumpToMaterial={onJumpToMaterial} />
        </div>
      ...
      {page.difficultyLevel && (
        ...
        >
          {page.difficultyLevel === 'foundational' && 'საფუძვლები'}
          {page.difficultyLevel === 'intermediate' && 'საშუალო'}
          {page.difficultyLevel === 'advanced' && 'მაღალი'}
          {page.difficultyLevel === 'synthesis' && 'სინთეზი'}
        </span>
      )}
    </div>
  );
...
function AdvanceCta({
    ...
    return (
      ...
      >
        ...
        <p className="text-sm font-bold tracking-tight" style={{ fontFamily: 'var(--font-display)' }}>
          {hasChecks ? 'შემოწმე გაგება ჩაკეტილია' : 'შემდეგი გვერდი ჩაკეტილია'}
        </p>
        <p className="text-xs text-muted-foreground max-w-sm mx-auto leading-relaxed">
          ისაუბრე Walli-სთან მასალაზე — როცა მზად იქნები, ის გახსნის შემდეგ ეტაპს.
        </p>
  ...
  return (
    ...
    >
      ...
      {hasChecks ? 'შემოწმე გაგება' : 'გავიგე — შემდეგი'}
      <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-0.5" />
    </motion.button>
  );
}
